...
"""
Generate C++ header for Rust ChaosMode implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_chaosmode')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_chaosmode', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
