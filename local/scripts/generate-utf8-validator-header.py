...
"""
Generate C++ header for Rust UTF-8 Validator implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_utf8_validator')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_utf8_validator', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
