...
"""
Generate C++ header for Rust ArrayUtils implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_arrayutils')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_arrayutils', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
