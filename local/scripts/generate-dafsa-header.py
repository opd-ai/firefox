...
"""
Generate C++ header for Rust Dafsa implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_dafsa')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_dafsa', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
