...
"""
Generate C++ header for Rust XorShift128+ implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_xorshift128plus')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_xorshift128plus', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
