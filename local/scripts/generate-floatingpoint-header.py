...
"""
Generate C++ header for Rust FloatingPoint implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_floatingpoint')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_floatingpoint', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
