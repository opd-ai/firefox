...
"""
Generate C++ header for Rust Observer Array implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_observer_array')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_observer_array', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
