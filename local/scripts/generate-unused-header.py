...
"""
Generate C++ header for Rust Unused implementation using cbindgen.
...
def main(output, *inputs):
    ...
    crate_dir = os.path.join(repo_root, 'local', 'rust', 'firefox_unused')
    
    # Run cbindgen
    try:
        result = subprocess.run(
            [cbindgen, '--config', 'cbindgen.toml', '--crate', 'firefox_unused', 
             '--output', output],
            cwd=crate_dir,
            check=True,
            capture_output=True,
